...
import 'package:flutter_datetime_picker/flutter_datetime_picker.dart' as datetime;
...
class ResourceViewState extends State<ResourceView> {
  ...
  getusers() async {
    ...
    if (response.statusCode == 200) {
      ...
      if (userData['job'] == 'admin') {
        lesuser = lesuser.where((lesuser) => lesuser.created_by == userData['id']).toList();
      } else {
        lesuser = ll.map<User>((json) => User.fromJson(json)).toList();
      }

      setState(() {
        nomusers = lesuser.map((lesuser) => lesuser.firstname + ' ' + lesuser.lastname).toList();
      });
    }
  ...
  getplannings(iduser) async {
    ...
    if (response.statusCode == 200) {
      ...
      setState(() {
        tousplannings = tousplannings.where((tousplannings) => tousplannings.user_id == iduser).toList();
      });
    }
...
//   }
    ...
    return ListView.builder(
        ...
        itemBuilder: (_, index) {
          tousplannings.sort((a, b) {
            ...
            return DateTime.parse(a.start_time.toString()).compareTo(DateTime.parse(b.start_time.toString()));
          });
          print(DateFormat('yyyy-MM-dd').format(tousplannings[index].start_time));
          print(DateFormat('yyyy-MM-dd').format(_selectedDate));
          if (DateFormat('yyyy-MM-dd').format(tousplannings[index].start_time) ==
              DateFormat('yyyy-MM-dd').format(_selectedDate)) {
            String? usss;
            //  for(int i =0 ; i< lesuser.length ; i++ ){
            //     List iduser = lesuser
            ...
            for (int i = 0; i < activites.length; i++) {
              List idactivites = activites.where((activites) => activites.id == tousplannings[index].activity).toList();
            ...
            content: Row(
              children: [
                /*Container(
                    height: 50,
                    width: 120,
                    decoration: BoxDecoration(color: Color.fromARGB(255, 255, 0, 0), borderRadius: BorderRadius.circular(30)),
                    child: FlatButton(
                ...
                Container(
                    height: 50,
                    width: 120,
                    decoration: BoxDecoration(color: Color.fromARGB(255, 0, 255, 42), borderRadius: BorderRadius.circular(30)),
                    child: FlatButton(
                ...
                Row(
                  ...
                  children: [
                    Container(
                      ...
                      child: MyButton(label: "Update Task", ontap: () => {update(_id, plannings)}),
                    )
                  ],
                ),
                Row(
  ...
  _getDateFromUser() async {
    DateTime? _pickerDate =
        await showDatePicker(context: context, initialDate: DateTime.now(), firstDate: DateTime(2015), lastDate: DateTime(2055));
  ...
  _showTimePicker() {
    return datetime.DatePicker.showDateTimePicker(context,
        ...
        theme: datetime.DatePickerTheme(
  ...
  void update(int _id, Planning plannings) async {
    ...
    for (int i = 0; i < lesuser.length; i++) {
      List iduser = lesuser
          .where((lesuser) => lesuser.firstname == user.text.split(' ')[0] && lesuser.lastname == user.text.split(' ')[1])
          .toList();
      user_id = iduser[0].id;
    }
    int? activite_id;
    for (int i = 0; i < activites.length; i++) {
      List idactivites = activites.where((activites) => activites.name == activity.text).toList();
